...
def test_analyze_valid_content(analyzer, sample_log_content):
    """Test analyzing valid log content"""
    result = analyzer.analyze(sample_log_content)
    
    assert result['total_requests'] == 23
    assert result['unique_ip_count'] == 11
    assert '177.71.128.21' in result['unique_ips']
    assert '168.41.191.40' in result['unique_ips']


def test_top_urls(analyzer, sample_log_content):
    """Test top URL calculation"""
    result = analyzer.analyze(sample_log_content)
    
    top_urls = result['top_urls']
    assert len(top_urls) > 0
    assert top_urls[0]['value'] == '/docs/manage-websites/'
    assert top_urls[0]['count'] == 2


def test_top_ips(analyzer, sample_log_content):
    """Test top IP calculation"""
    result = analyzer.analyze(sample_log_content)
    
    top_ips = result['top_ips']
    assert len(top_ips) > 0
    assert top_ips[0]['value'] == '168.41.191.40'
    assert top_ips[0]['count'] == 4